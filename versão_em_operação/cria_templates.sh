...
cria_template() {
cat << EOF > ${4}
dset /stornext/online8/bamc/w.santos/Experimento_umidade_do_solo/novos_experimentos_nova_umidade/${1}/GPOSCPT${1}%y4%m2%d2%h2P.fct.TQ0299L064.grb
*
index ^GPOSCPT${1}.idx
*
undef -9.99e+08
*
title PRESSURE HISTORY    PTEC AGCM REVIS 1.0 2000  T029964   COLD
*
dtype grib   255
*
...
tdef   ${2} linear ${3} 6hr
*
zdef    33 levels  1000  985  975  960  950  930  925  915  900  880
                  875  850  825  800  775  750  725  700  675  650
                  600  500  300  250  200  150  100   70   50   30
                   20   10    3
vars    55
...
TPOR   33   17,  100,    0  **        DEW POINT TEMPERATURE                   (K               )
...
for data in $(find . -maxdepth 1 -type d -name "2*")
do 

  data=${data:2:12}

  tdef=$(ls ${data}/*grb | wc -l)

  datafmt=$(${inctime} ${data} +0d %h2Z%d2%MC%y4)

  echo "${data} - ${tdef} - ${datafmt}"

  arqctl="./templates/GPOSCPT${data}.ctl"

  cria_template ${data} ${tdef} ${datafmt} ${arqctl}

  gribmap -v -e -i ${arqctl} 
