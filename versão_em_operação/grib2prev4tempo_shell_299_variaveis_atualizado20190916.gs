...
mestwo = JAN
*dayinicial = 1
dayinicial = totalmes - 6
prev = 168

dir = 'templates'
*dir = '/stornext/online8/bamc/w.santos/Experimento_umidade_do_solo/Experimento_com_GL_SM/bam_pos_dataout/TQ0299L064'

*OBS>
*nao esquecer de modificar no table la abaixo
*rc = write(dummy.ctl,'tdef 25  linear 'HH':00Z01'mestwo''yeartwo' 24hr',append)
...
*Buscando dados no dir online8
* 'open 'dir'/'ano''mes''dayinicial''HH'/GPOSCPT'ano''mes''dayinicial''HH'.ctl'
*  say ''dir'/'ano''mes''dayinicial''HH'/GPOSCPT'ano''mes''dayinicial''HH'.ctl'

  'open 'dir'/GPOSCPT'ano''mes''dayinicial''HH'.ctl'
  say ''dir'/GPOSCPT'ano''mes''dayinicial''HH'.ctl'
...
*Gravando dados no dir online8
*'set fwrite /stornext/online8/mcga/w.santos/TQ0299L064/'name'.bin'
*say 'set fwrite /stornext/online8/mcga/w.santos/TQ0299L064/'name'.bin'

*Gravando dados no dir online8
'set fwrite 'dir'/'name'.bin'
say 'set fwrite 'dir'/'name'.bin'
...
**
  ...
  while(i<=totalmestwo)
*  while(i<=totalmestwo & datafi<=31 & arq<=31 )
*  while(i<=totalmestwo & datafi<=31 )
    ...
    say 'totalmes' totalmes
    say 'gravando variavel 1 nivel'  
    say 'datafi'datafi
    say 'monthout'monthout
    say 'yeartwo'yeartwo    
    say 'arq'arq
...
*modifiquei o undef por conta de um erro que ocorria com uma data apenas
...
*fixei a data de 25 por questao de erro que nao consegui solucionar
*rc = write(dummy.ctl,'tdef 24  linear 'HH':00Z01'mestwo''yeartwo' 24hr',append)
rc = write(dummy.ctl,'*',append)
...
***
rc = write(dummy.ctl,'vars    8',append)
rc = write(dummy.ctl,'PREC    0   61,    1,    0  ** sfc    TOTAL PRECIPITATION                     (KG/M2/DAY       )',append)
rc = write(dummy.ctl,'USSL    0  182,    1,    0  ** sfc    SOIL WETNESS OF SURFACE                 (0-1             )',append)
rc = write(dummy.ctl,'UZRS    0  183,  112,    0  ** landt  SOIL WETNESS OF ROOT ZONE               (0-1             )',append)
rc = write(dummy.ctl,'UZDS    0  184,  112,    0  ** landt  SOIL WETNESS OF DRAINAGE ZONE           (0-1             )',append)
...
***
rc = close(dummy.ctl)
*modificando ou movendo para AGCM
*'!mv dummy.ctl /stornext/home/w.santos/scratchout/agcm_cptec/pos/dataout/TQ0299L064/'name'.ctl'

*modificando ou movendo para SMG
*'!mv dummy.ctl /stornext/home/w.santos/scratchout/SMG/datainout/bam/pos/dataout/TQ0299L064/'name'.ctl'

*modificando ou movendo para online8
*'!mv dummy.ctl /stornext/online8/mcga/w.santos/TQ0299L064/'name'.ctl'
*modificando ou movendo para online6
'!mv dummy.ctl 'dir'/'name'.ctl'
